...
export function Profile() {
  ...
  const menuItems = [
    { id: 'account',       icon: Settings,    label: t('account_settings'),  color: 'text-primary' },
    { id: 'notifications', icon: Bell,        label: t('notifications'),     color: 'text-secondary' },
    { id: 'privacy',       icon: Shield,      label: t('privacy_security'),  color: 'text-tertiary' },
    { id: 'payment',       icon: CreditCard,  label: t('payment_methods'),   color: 'text-primary' },
  ] as const;
  ...
  return (
    <div className="bg-surface font-body text-on-surface min-h-screen pb-32">
      <Header
        petName={isEditing ? formData.petName : (user?.petName || t('your_pet'))}
              ...
              {!isEditing ? (
                ...
                >
                  <div className="space-y-1">
                    <h2 className="text-3xl font-extrabold tracking-tight text-on-surface">
                      {user?.displayName || t('owner')} & {user?.petName || t('your_pet')}
                    </h2>
                    <p className="text-on-surface-variant font-medium text-lg leading-none">
                      {user?.petBreed ? `${user.petBreed} • ` : ''}{user?.petAge ? `${user.petAge} ${t('years')}` : t('setup_pending')}
                    </p>
                  </div>
                  <button
                ...
                >
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-black text-primary uppercase tracking-widest ml-1">{t('your_name')}</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-xs font-black text-primary uppercase tracking-widest ml-1">{t('dog_name')}</label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-black text-primary uppercase tracking-widest ml-1">{t('breed')}</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="text-xs font-black text-primary uppercase tracking-widest ml-1">{t('age')}</label>
                        <input
                          type="number"
            ...
            {!isEditing && (
              <>
                ...
                <section className="space-y-4">
                  ...
                  <p className="text-center text-xs font-bold text-on-surface-variant uppercase tracking-widest">{t('version')}</p>
                </section>
              </>
            )}
          </>
